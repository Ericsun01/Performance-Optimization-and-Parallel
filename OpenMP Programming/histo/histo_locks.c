...
long * histogram(char * fn_input) {
  ...
  for (m = 0; m < 100; m++) {
#pragma omp parallel for default(shared) private(i, j, tmp) firstprivate(image)
    for (i = 0; i < image->row; i++) {
      for (j = 0; j < image->col; j++) {
        // reduce the operations in critical section
        tmp = image->content[i][j];
        omp_set_lock(&(locks[tmp]));
        histo[tmp]++;
        omp_unset_lock(&(locks[tmp]));
      }
    }
  }
