...
int hypre_CSRMatrixMatvec(double alpha,
  ...
  if (num_rownnz < xpar * (num_rows)) {
    /*
          ...
          * } */
    if (num_vectors == 1) {
#pragma omp parallel for default(shared) private(i, j, jj, tempx)  // 1
...
#pragma omp parallel for default(shared) private(i, j, jj, tempx)  // 1
    ...
    if (num_vectors == 1) {
#pragma omp parallel for default(shared) private(i, j, jj, temp)  // 2
...
int hypre_CSRMatrixMatvecT(double alpha,
  ...
  if (num_threads > 1) {
#pragma omp parallel for default(shared) private(i1, i, j, jj, jv, ns, ne)  // 3
...
#pragma omp parallel for default(shared) private(i, jj, j, jv)  // 4
    for (i = 0; i < num_rows; i++) {
      if (num_vectors == 1) {
        for (jj = A_i[i]; jj < A_i[i + 1]; jj++) {
          j = A_j[jj];
...
#pragma omp parallel for default(shared) private(i, jj) reduction(+ : temp)  // 5
  for (i = 0; i < num_rows; i++) {
    if (CF_marker_x[i] == fpt) {
      temp = y_data[i];
      for (jj = A_i[i]; jj < A_i[i + 1]; jj++)
